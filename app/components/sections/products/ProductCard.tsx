...
function ImageChips({ product }: { product: Product }) {
  return (
    <div className="pointer-events-none absolute inset-x-5 top-5 z-10 flex items-start justify-between gap-2">
      <span className="rounded-full bg-black/45 px-2.5 py-1 text-[10.5px] font-medium uppercase tracking-[0.08em] text-[#0a0a0a]/85 backdrop-blur-md">
        {product.material}
      </span>
      {product.badge ? (
        <Badge
  ...
  const media = (
    <div
      className={cn(
        ...
        product.darkShot
          ? "bg-gradient-to-b from-[#ececea] to-[#f6f5f2]"
