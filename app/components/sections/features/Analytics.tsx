...
function DashboardMock() {
  ...
  return (
    <Tilt
      max={5}
      className="mx-auto w-full max-w-md rounded-[24px] bg-[#f6f5f2] p-7 shadow-[0_40px_80px_-30px_rgba(0,0,0,0.4)] ring-1 ring-black/[0.04]"
    >
      <div className="flex items-center gap-5 border-b border-black/[0.06] pb-6">
        <ActivityRing
          size={92}
          strokeWidth={9}
          gap={4}
        ...
        <div className="grid flex-1 grid-cols-3 gap-3">
          {[
            ...
            <div key={k}>
              <p className="font-display text-3xl font-extrabold text-[#0a0a0a]">{v}</p>
          ...
          <div className="mt-4 flex h-24 items-end gap-1.5">
            {BARS.map((h, i) => (
              <span
                ...
                style={{
                  height: `${h}%`,
                  backgroundColor: i >= BARS.length - 2 ? "#e5242a" : "#d8d7d4",
                }}
              />
            ))}
          </div>
            ...
            {countries.map(([name, pct, w]) => (
              <div key={name}>
                ...
                <div className="mt-1.5 h-1 w-full rounded-full bg-black/[0.08]">
                  <span
                    className="block h-full rounded-full bg-[#e5242a]"
