...
function Icon({ children }: { children: ReactNode }) {
  return (
    <span className="flex size-10 shrink-0 items-center justify-center rounded-xl border border-black/[0.06] bg-black/[0.03]">
...
function FinishVisual() {
  return (
    <Float amount={10} duration={7}>
      <Tilt
        max={8}
        className="relative mx-auto aspect-square w-full max-w-[420px] overflow-hidden rounded-[28px] bg-gradient-to-br from-[#ececea] to-[#f6f5f2] ring-1 ring-black/[0.08]"
      >
        <Media
        ...
        <div className="pointer-events-none absolute bottom-5 left-5 flex items-center gap-2.5">
          <span className="size-7 rounded-full bg-[#d8d7d4] ring-1 ring-black/10" />
          <span className="size-7 rounded-full bg-gradient-to-b from-[#d0d0d0] to-[#8a8a8a] ring-1 ring-black/20" />
          <span className="size-7 rounded-full bg-[#ececec] ring-1 ring-black/20" />
    ...
    <section className="bg-[#ffffff] py-24">
      <Container size="lg">
        <Reveal>
          ...
          <h2 className="font-display mt-4 text-balance text-4xl font-extrabold leading-[1.05] tracking-tight text-[#0a0a0a] sm:text-5xl md:text-[56px]">
            ...
            {ITEMS.map((it) => (
              <StaggerItem key={it.title}>
                <div className="flex items-start gap-4">
                  <Icon>{it.icon}</Icon>
                  <div>
                    <p className="text-[15px] font-bold text-[#0a0a0a]">{it.title}</p>
