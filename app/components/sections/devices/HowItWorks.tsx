        ...
        <Stagger className="mt-14 grid gap-8 sm:mt-16 md:grid-cols-3">
          {STEPS.map((step, i) => (
            <StaggerItem key={step.title} className="flex flex-col">
              <div className="relative aspect-square overflow-hidden rounded-3xl border border-border bg-[#f6f5f2]">
                <Media
                  src={step.image.src}
                  alt={step.image.alt}
                  fill
                ...
                />
                <span className="absolute left-5 top-5 flex size-9 items-center justify-center rounded-full border border-black/15 bg-black/40 font-display text-sm font-semibold text-[#0a0a0a] backdrop-blur">
                  {i + 1}
                </span>
              </div>
